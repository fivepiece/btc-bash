...
readarray inputs < <( testnet-cli listunspent | grep "txid\|vout\|address\|scriptPubKey\|redeemScript\|amount\|}\|{" )
    ...
    do
        ...
        case "${field}" in
            ...
            *address*)
                ...
                read key < <( testnet-cli dumpprivkey "${field}" )
                if (( ${j} == 0 ))
                then
...
addr2pubkey() {
    ...
    local pubkey
    read pubkey < <( testnet-cli validateaddress "${1}" | grep pubkey )
    pubkey="${pubkey#*:}"
    pubkey="${pubkey//[\",]/}"
...
mkrandouts() {
    ...
    declare -A outputs
#    read balance < <( testnet-cli getbalance "" 1 )
        ...
        case ${thisscript} in
            ...
            *mofn*)
                ...
                do
                    ...
                    # testnet-cli importpubkey "${pubkey}" '' false 2>/dev/null
                    pubkeys+=( "${pubkey}" )
                    # echo "pubkey    : ${pubkey}"
                    i=$(( ${i}+1 ))
                done
                ...
                # eval "testnet-cli addmultisigaddress "${m}" '${pubkeys_json}'"
            ...
            *p2wsh*)
                ...
                # testnet-cli importaddress "${script}" "" false true
                # testnet-cli importaddress "${witaddr}" "" false false
            ...
            p2sh*)
                ...
                if [[ "${thisscript}" =~ "p2w" ]]
                then
                    testnet-cli importaddress "${witaddr}" "" false false 2>/dev/null
                fi
                ...
                read hexscript < <( serscript "${data[*]}" )
                read addr < <( testnet-cli decodescript "${script}" | grep p2sh )
                addr="${addr#*: \"}"
                addr="${addr%\"}"
...
#    set -e
    create="testnet-tx -create ${thisoutput[@]}"
    read newtx < <( eval ${create} )
#    echo ${newtx} > "${HOME}/Documents/fundme"

#    fund="testnet-cli fundrawtransaction ${newtx} | grep -- \"hex\|fee\""
#    echo "funding..."
    echo "${newtx}" > "${HOME}/Documents/thisfund" 
#    fund=" echo "${newtx}" | testnet-cli -stdin fundrawtransaction '{\"lockUnspents\":true}' | grep -- \"hex\|fee\"" 
#    echo "funded?"
...
#        readarray -t fundstate < <( eval ${fund} 2>&1 )
    readarray -t fundstate < <( cat "${HOME}/Documents/thisfund" | testnet-cli -stdin fundrawtransaction | grep -- "hex\|fee" 2>&1 )
#    echo "${fundstate[*]}"
    ...
    then
        echo "-32603"
        read unspent < <( testnet-cli listunspent 2 3 | grep amount -c )
        if (( ${unspent} == 0 ))
        then
            echo "no inputs"
            read
...
#    sign="testnet-cli signrawtransaction ${ustx} | grep hex"
    read sntx < <( echo "${ustx}" | testnet-cli -stdin signrawtransaction | grep "hex" )
#    read sntx < <( eval ${sign} )
    sntx="${sntx#*: \"}"
    sntx="${sntx%*\",}"
...
genoutaddrs() {
    ...
    testnet-cli dumpwallet "${PWD}/wallet.txt"
...
#    return
    ...
    read balance < <( testnet-cli getbalance "*" 1 )
    read unspent < <( testnet-cli listunspent 1 | grep amount -c )
    ...
    do
        ...
        then
            ...
            lastbalance="${balance}"
            read balance < <( testnet-cli getbalance \'*\' 1 )
            read unspent < <( testnet-cli listunspent 1 | grep amount | grep -v ' 0\.0' -c )
